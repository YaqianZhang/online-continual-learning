...
class ER_dyna_iter_aug_dbpg(ExperienceReplay):
    def __init__(self, model, opt, params):
        super(ER_dyna_iter_aug_dbpg, self).__init__(model, opt, params)
        ...
        aug_map=[(1,5),(1,14),(2,5),(2,14),(3,5),(3,14),(4,5),(4,14)]
        aug_map.reverse()
        iter_map=[1,5,10,15,20]
        self.action_map = []
        for aug in aug_map:
    ...
    def update_weight_bpg(self,train_acc_list,STOP_FLAG,action_id):
        current_iter = len(train_acc_list)
        if(current_iter ==0 or current_iter == None):
            return self.params.mem_iters
        ...
        if(last_acc>target_acc_end or STOP_FLAG  ): ## too large
    ...
    def sample_action_bpg(self):
        ...
        return self.action_map[action_idx] ,action_idx ## choice [action]
    def train_learner(self, x_train, y_train):
        ...
        for ep in range(self.epoch):
            for i, batch_data in enumerate(train_loader):
                ...
                [N,M,memiter],action_id = self.set_dyna_iter(self.memory_manager.current_performance,STOP_FLAG)
                ...
                self.mem_iter_list.append(memiter)
                logs = self.update_weight_bpg(train_acc_list,STOP_FLAG,action_id)
                if(train_stats!= None):
                ...
                if i % 100 == 1 and self.verbose:
                    ...
                    print("memiter",memiter,"aug",aug_strength)#np.max(train_acc_list),train_acc_list[-1],np.mean(train_acc_list))
