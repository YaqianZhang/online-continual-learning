...
class SCR_spread(SupContrastReplay):
    def __init__(self, model, opt, params):
        super(SCR_spread, self).__init__(model, opt, params)
    ...
    def perform_scr_update(self,combined_batch, combined_labels,mem_num):
        ...
        fake_labels = torch.arange(0,len(combined_labels))

        self_sup_loss,_ =self.criterion(features, fake_labels)
        ...
        return loss.item(),self.self_sup_beta*self_sup_loss.item()
        ...
        for ep in range(self.epoch):
            for i, batch_data in enumerate(train_loader):
                ...
                if i % 100 == 1 and self.verbose:
                    ...
                    #         .format(i, scr_loss,)
                    # )
                    print("==>> it: {:},scr loss: {:.2f}, self_sup loss: {:.2f}".format(i,scr_loss,self_sup_loss,acc_mem))
