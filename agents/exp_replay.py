...
class ExperienceReplay(ContinualLearner):
    ...
    def _batch_update(self,batch_x,batch_y,losses_batch,acc_batch,i,replay_para=None,mem_num=0):
        ...
        if(mem_num>0):
            self.predict_label_list.append(pred_label[:mem_num].detach().cpu())
            self.batch_y_list.append(batch_y[:mem_num].cpu())
    ...
    def train_learner(self, x_train, y_train): ## todo: move build of dataloader from agent to dataset script
        ...
        for ep in range(self.epoch):
            for i, batch_data in enumerate(train_loader):
                ...
                end_time = time.time()
                if i % 500 == 1 and self.verbose and ep % 10 == 0:
                    print(
                        '==>>> it: {}, avg. loss: {:.2f}, '
                    ...
                    print("mem_iter: {}, time: {:.2f}"
                          .format(memiter,end_time-start_time))
        self.after_train()
