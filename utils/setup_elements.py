...
transforms_match = {
    ...
    "imagenet1000":transforms.Compose([
        transforms.ToTensor(),
        ]),
...
def setup_architecture(params):
    ...
    if params.agent in ["SCR_der","SCR_spread","SCR_spread_class",'SCR','SCR_META', 'SCP',"SCR_RL_ratio","SCR_RL_iter"]:
        if params.data == 'mini_imagenet':
            ...
            if(params.resnet_size == "normal"):
                ...
                return SupConResNet( 160,head=params.head)
    ...
    if params.data == 'cifar100':
        ...
        return model
    elif params.data == 'imagenet1000':
        if (params.resnet_size == "normal"):
            model = ResNet18(nclass)
            model.linear = nn.Linear(25088, nclass, bias=True)
        else:
            model = Reduced_ResNet18(nclass)
            model.linear = nn.Linear(2560, nclass, bias=True)
