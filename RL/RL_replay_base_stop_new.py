...
class RL_replay(object):
    ...
    def __init__(self,params,close_loop):
        ...
        self.ob_dim = 5 #4 ## train stats #self.RL_env.get_state_dim()
        action_num = params.mem_iter_max  +1 #stop / continue   ##self.RL_env.get_action_dim()
        if(self.params.RL_type == "RL_MDP"):
            self.RL_agent = RL_DQN_agent_hp(params, action_num, self.ob_dim, RL_replay=self)
        elif(self.params.RL_type == "RL_MAB"):
            self.RL_agent = RL_MAB_agent(params,action_num,self.ob_dim, RL_replay=self)
        elif(self.params.RL_type =="RL_MDP_stop"):

            self.RL_agent = RL_MDP_agent_stop(params, action_num, self.ob_dim, RL_replay=self)
        else:
            self.RL_agent = None
    ...
    def set_immediate_reward(self,train_stats, STOP_FLAG):
        ...
        elif(self.params.immediate_reward == "relative_train_acc_mem"):
    ...
    def set_end_reward(self,train_stats,j):
