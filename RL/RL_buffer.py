...
class RL_ExperienceReplay(object):
    ...
    def reset_RL_buffer(self):
        ...
        self.next_state_list=[] ## to-do: more efficient storage with one state_list
        self.done_list=[]
    def save_buff(self,prefix):
        arr = np.array(self.state_list)

        np.save(prefix + "buffer_state_list.npy", arr)

        arr = np.array(self.action_list)

        np.save(prefix + "buffer_action_list.npy", arr)

        arr = np.array(self.reward_list)

        np.save(prefix + "buffer_reward_list.npy", arr)
